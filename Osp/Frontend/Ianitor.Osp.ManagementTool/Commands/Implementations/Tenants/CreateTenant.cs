...
{
    internal class CreateTenant : ServiceClientOspCommand<ICoreServicesClient>
    {
        private IArgument _tenantIdArg;
        private IArgument _databaseArg;

        public CreateTenant(IOptions<OspToolOptions> options, ICoreServicesClient coreServicesClient, IAuthenticationService authenticationService)
            : base("Create", "Creates a new tenant.", options, coreServicesClient, authenticationService)
        {
        }
